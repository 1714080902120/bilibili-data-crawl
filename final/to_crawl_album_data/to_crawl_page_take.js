...
const { saveData } = require('../save_to_db')
  ...
  ; (async function () {
    ...
    await page.goto('https://h.bilibili.com/ywh/h5/home#/photo', { waitUntil: 'networkidle2' })
    ...
    for (let i = 0; i < link.length; i++) {
      let location = `E:/VScode/bilibili/bilibili_data/album_data/take_data/${link[i].mid}/`
  ...
  console.log(page_take.length)
  
  page_take = await page_take.filter(n => {
    let reg = /http/gi
    return reg.test(n.img.originSrc)
  })
  console.log(page_take.length)

  for (let i = 0; i < page_take.length; i++) {
    for (let j = i + 1; j < page_take.length; j++) {
      if (page_take[i].link.mid === page_take[j].link.mid) {
        page_take.splice(j, 1)
        break
      }
    }
  }
  ...
  for (let i = 0; i < page_take.length; i++) {
    let location = `E:/VScode/bilibili/bilibili_data/album_data/take_data/${page_take[i].link.mid}/`    
    await fs.mkdir(location, err => { if (err) console.log(err) })
  }

  for (let j = 0; j < page_take.length; j++) {
    let location = `E:/VScode/bilibili/bilibili_data/album_data/take_data/${page_take[j].link.mid}/`
    // 下载文件夹
    // await fs.mkdir(location, err => { if (err) console.log(err) })
    // 图片
    await c.queue({
      url: page_take[j].img.originSrc,
      filename: page_take[j].img.src + page_take[j].img.name,
      headers
    })
    // logo
    await c.queue({
      url: page_take[j].up.logo.originSrc,
      filename: page_take[j].up.logo.src + page_take[j].up.logo.name,
      headers
    })
  }
  
  await saveData(page_take, 'album_take_data')
  })()
