...
const { saveData } = require('../save_to_db')
  ...
  ; (async function () {
    ...
    await page.goto('https://m.bilibili.com/channel/188', { waitUntil: 'networkidle2' })
    ...
    for (let i = 0; i < type.length - 2; i++) {
      let location = `E:/VScode/bilibili/bilibili_data/digital_data/${type[i].name}/`
      let obj = {}
      ...
      for (let j = 0; j < content.link[i].length; j++) {
        let itemLocation = `E:/VScode/bilibili/bilibili_data/digital_data/${type[i].name}/${content.link[i][j].bvid}/`
    ...
    await saveData(arr, 'digital_data')
