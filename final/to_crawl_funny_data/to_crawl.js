...
const { saveData } = require('../save_to_db')
  ...
  ; (async function () {
    ...
    await page.goto('https://m.bilibili.com/channel/5', { waitUntil: 'networkidle2' })
    ...
    let content = await (async function () {
      ...
      // 获取封面
      await page.waitForSelector('.card-box img')
      let img = await page.$$eval('.card-box img', elements => {
        let a = []
        let b = []
    ...
    for (let i = 0; i < type.length - 2; i++) {
      let location = `E:/VScode/bilibili/bilibili_data/funny_data/${type[i].name}/`
      let obj = {}
      ...
      // 下载文件夹
      // await fs.mkdir(loc/ation, err => { if (err) console.log(err) })
    

      // 下载图片及文件所在的文件夹
      for (let j = 0; j < content.link[i].length; j++) {
        let itemLocation = `E:/VScode/bilibili/bilibili_data/funny_data/${type[i].name}/${content.link[i][j].bvid}/`
